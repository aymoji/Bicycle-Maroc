                        ...
                        <a href="{{ route('bikes.index') }}">
                            <div class="flex items-center p-4 bg-white rounded-lg shadow-xs  hover:bg-pr-200 ">
                                ...
                                <div>
                                    ...
                                    <p class="text-lg font-semibold text-gray-700 dark:text-gray-200">
                                        {{ $bikes->where('status', 'Available')->count() }}
                                    </p>
                                </div>
                            </div>
                        </a>
                                    ...
                                    @forelse ($reservations as $reservation)
                                        <tr class="text-gray-700 dark:text-gray-400">
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                {{ $reservation->bike->brand }} {{ $reservation->bike->model }}

                                            </td>

                                            <td class="px-4 py-3  text-sm">
                                                {{ Carbon\Carbon::parse($reservation->start_hour)->format('y-m-d') }} </td>
                                            <td class="px-4 py-3  text-sm">
                                                {{ Carbon\Carbon::parse($reservation->end_hour)->format('y-m-d') }} </td>

                                            <td class=" text-xs">
                                                <p class="px-4 py-3 text-sm">
                                                    {{ Carbon\Carbon::parse($reservation->end_hour)->diffInHours(Carbon\Carbon::parse($reservation->start_hour)) }}
                                                    hours </p>
                                            </td>


                                            <td class="px-4 py-3 text-xs">
                                                @if ($reservation->start_hour > Carbon\Carbon::now())
                                                    <p class="px-4 py-3 text-sm">
                                                        {{ Carbon\Carbon::parse($reservation->end_hour)->diffInHours(Carbon\Carbon::now()) }}
                                                        hours
                                                    </p>
                                                @else
                                            ...
                                            <td class="px-4 py-3 text-sm">
                                                {{ $reservation->bike->price_per_hour * $reservation->hours }} $
                                            </td>
